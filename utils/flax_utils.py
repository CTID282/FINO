...
class TrainState(flax.struct.PyTreeNode):
    ...
    def load(self, load_dict):
        opt_state = self.tx.init(load_dict)
        return self.replace(
            params=load_dict,
            opt_state=opt_state
        )
...
def save_agent(agent, save_dir, epoch):
    ...
    Args:
        ...
        epoch: Epoch number.
    """
    save_path = os.path.join(os.getcwd(), save_dir, f'{epoch}')
    for name in agent.network.params.keys():
        save_name = name.split('modules_')[1]
        save_name = os.path.join(save_path, save_name)
        orbax_checkpointer = orbax.checkpoint.PyTreeCheckpointer()
        ckpt = {'model': agent.network.params[name]}
        save_args = orbax_utils.save_args_from_target(ckpt)
        orbax_checkpointer.save(save_name, ckpt, save_args=save_args, force=True)
...
def restore_agent(agent, restore_path, restore_epoch):
    ...
    """
    load_dict = dict()
    load_path = os.path.join(os.getcwd(), restore_path, f'{restore_epoch}')
    for name in agent.network.params.keys():
        load_name = name.split('modules_')[1]
        load_name = os.path.join(load_path, load_name)
        orbax_checkpointer = orbax.checkpoint.PyTreeCheckpointer()
        target = {'model': agent.network.params[name]}
        load_model = orbax_checkpointer.restore(load_name, item=target)
        load_dict[name] = load_model['model']
    network = agent.network.load(load_dict)
    agent = agent.replace(network=network)
    return agent