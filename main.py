...
flags.DEFINE_integer('buffer_size', 1500000, 'Replay buffer size.')
...
flags.DEFINE_string('project_name', None, 'Wandb project name.')

config_flags.DEFINE_config_file('agent', 'agents/fino.py', lock_config=False)
...
def main(_):
    ...
    exp_name = f"{job_type}_{FLAGS.seed}"
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps + FLAGS.online_steps + 1), smoothing=0.1, dynamic_ncols=True):
        ...
        if FLAGS.eval_interval != 0 and (i == 1 or i % FLAGS.eval_interval == 0):
            ...
            if hasattr(agent, 'update_entropy') and (i > FLAGS.offline_steps):
                agent, entropy = agent.update_entropy(batch['observations'], jax.random.PRNGKey(FLAGS.seed))
                eval_metrics['evaluation/entropy'] = entropy
                if hasattr(agent, 'beta'):
                    eval_metrics['evaluation/beta'] = agent.beta
